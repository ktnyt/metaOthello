// This file is provided as a part of MetaOthello. Please refer to LICENSE.TXT
// for the 'License'
#pragma once
/*!
 \file othello.h
...
/*!
 * \brief Describes the data structure *l-Othello*. It classifies keys of
 * *keyType* into *2^L* classes. \note Query a key of keyType always return
 * uint64_t, however, only the lowest L bits are meaningful. \n The array are
 * all stored in an array of uint64_t. There are actually m_a+m_b cells in this
 * array, each of length L.
 */
template <class keyType>
class Othello {
  typedef uint64_t valueType;
#define MAX_REHASH \
  20  //!< Maximum number of rehash tries before report an error. If this limit
      //!< is reached, Othello build fails.
 public:
  vector<valueType> mem;  //!< actual memory space for arrayA and arrayB.
  uint32_t L;             //!< the length of return value.
//    uint32_t LMASK;  return value must be within [0..2^L-1], i.e.,
//    LMASK==((1<<L)-1);
#define LMASK ((1ULL << L) - 1)
  ...
  vector<uint32_t> fillcount;  //!< Enabled only when the values of the query is
                               //!< not pre-defined. This supports othelloIndex
                               //!< query. fillcount[x]==1 if and only if there
                               //!< is an edge pointed to x,
#define FILLCNTLEN (sizeof(uint32_t) * 8)
  uint32_t allowed_conflicts;   //!< The number of keys that can be skipped
                                //!< during construction.
 ...
 private:
   ...
   * \warning May return garbage info on the higher bits, needs (& LMASK)
   * afterwards.
   */
  inline valueType get(uint32_t loc) {  //
    ...
    if (L & (L - 1)) {
      // L &(L-1) !=0 ==> L is NOT some power of 2 ==> value may cross the
      // uint64_t barrier.
      if ((mx + L) > 64) {
  ...
  /*!
   ...
   \param [in] void * values, pointer to the array of values. Each value is of
   *valuesize* bytes. \param [in] uint32_t keycount. \param [in] size_t
   valuesize. \note  When *values* is NULL, mark edges as 0 or 1 according to
   their direction.
 ...
 public:
  /*!
   ...
   \param [in] bool _autoclear, clear memory used during construction once
   completed. Forbid future modification on the structure. \param [in] void *
   _values, Optional, pointer to array of values. When *_values* is empty, fill
   othello values such that the query result classifies keys to 2 sets. See more
   in notes. \param [in] uint32_t valuesize, must be specifed when *_values* is
   not NULL. This indicates the length of a valueType; \param [in]
   _allowed_conflicts, default value is 10. during construction, Othello will
   remove at most this number of keys, instead of rehash. \note keycount should
   not exceed 2^29 for memory consideration. \n when *_values* is empty,
   classify keys into two sets X and Y, defined as follow: for each connected
   compoenents in G, select a node as the root, mark all edges in this connected
   compoenent as pointing away from the root. for all edges from U to V, query
   result is 1 (k in Y), for all edges from V to u, query result is 0 (k in X).

  */
  Othello(uint8_t _L, keyType *_keys, uint32_t keycount, bool _autoclear = true,
    ...
    while (((uint64_t)mem.size()) * sizeof(mem[0]) * 8ULL <
           (ma + mb) * ((uint64_t)L))
      mem.push_back(0);
    ...
    while ((!build) && (hl1 <= 31 && hl2 <= 31)) {
      ...
      if (!build) {
        ...
        cout << "Extend Othello Length to" << human(keycount)
             << " Keys, ma/mb = " << human(ma) << "/" << human(mb) << " keyT"
             << sizeof(keyType) * 8 << "b  valueT" << sizeof(valueType) * 8
             << "b"
             << " L=" << (int)L << endl;
        trycount = 0;
      }
    ...
    printf("%08x %08x\n", Ha.s, Hb.s);
    if (build)
      cout << "Succ " << human(keycount) << " Keys, ma/mb = " << human(ma)
           << "/" << human(mb) << " keyT" << sizeof(keyType) * 8 << "b  valueT"
           << sizeof(valueType) * 8 << "b"
  ...
  Othello(uint8_t _L, vector<keyType> &keys, vector<VT> &values,
          bool _autoclear = true, int32_t allowed_conflicts = -1)
      : Othello(_L, &(keys[0]), keys.size(), _autoclear, &(values[0]),
                sizeof(VT), allowed_conflicts) {}

  //! \brief release memory space used during construction and forbid future
  //! modification of arrayA and arrayB.
  void finishBuild() {
    delete nxt1;
    delete nxt2;
    delete first;
  ...
  /*!
      \brief export the information of the *Othello*, not including the array,
     to a memory space. \note memory space length = 0x20. \n Exported infomation
     contains:  \n 0x00, 32bit, L; \n 0x04, 32bit, seedB; \n 0x08, 32bit, seedA;
     \n 0x10, 8bit, hlA; 0x14, 8bit, hlB; seedA, seedB, ma , mb (represented as
     1<<hl1 and 1<<hl2).
  */
  ...
  */
  ...
  void printValueTSize() { std::cout << sizeof(valueType) << std::endl; }

  vector<uint32_t> getCnt();  //!< \brief returns vector, length = 2L. position
                              //!< x: the number of 1s on the x-th lowest bit,
  ...
  /*!
    ...
    return 1 with loest probability. \note This function is usually able to tune
    the probabilty within 0.2 ~ 0.8.
   */
  void setAlienPreference(double ideal = 1.0);

 private:
  void inline get_hash_1(const keyType &v, uint32_t &ret1) { ret1 = (Ha)(v); }
 ...
 public:
  ...
  /*!
   ...
   \note only the arrayA and B are loaded. This must be called after using
   constructor Othello<keyType>::Othello(unsigned char *)
   */
...
template<size_t L>
std::array<uint8_t,L> operator ^ (const std::array<uint8_t,L>  &A,const
std::array<uint8_t,L>  &B) { std::array<uint8_t, L> v = A; for (int i = 0; i <
L; i++) v[i]^=B[i]; return v;
...
bool Othello<keyType>::testHash(uint32_t keycount) {
  ...
  for (int i = 0; i < keycount; i++) {
    ...
    if (disj.sameset(ha, hb)) {
      ...
      if (removedKeys.size() > allowed_conflicts) return false;
      continue;
    }
    (*nxt1)[i] = (*first)[ha];
    (*first)[ha] = i;
...
template <class keyType>
void Othello<keyType>::fillvalue(void *values, uint32_t keycount,
                                 size_t valuesize) {
  list<uint32_t> Q;
  vector<int32_t> *nxt;
  filled.resize(ma + mb);
      ...
      while (!Q.empty()) {
        ...
        while (kid >= 0) {
          ...
          //                        valueKid = values[kid];
            ...
            //! when hthis == ha, this is a edge pointing from U to V, i.e.,
            //! value of this edge shall be set as 1.
            valueKid = (hthis == ha) ? 1 : 0;
            fillcount[helse / FILLCNTLEN] |= (1 << (helse % FILLCNTLEN));
          }

          valueType newvalue = valueKid ^ get(hthis);
          //                    printf("%x %x %x===", valueKid, get(hthis),
          //                    newvalue);
          set(helse, newvalue);
          //                    printf("k%llx ha/hb %lx %lx: %x ^ %x = %x ^ %x =
          //                    %x (%x), helse%x ,i%x, %d\n",
          //                       keys[kid], ha, hb, get(hthis) & LMASK,
          //                       newvalue & LMASK, get(ha) &LMASK, get(hb)
          //                       &LMASK, (get(ha)^get(hb)) & LMASK, valueKid
          //                       &LMASK, helse ,hthis ,(bool)((valueKid
          //                       &LMASK)==((get(ha)^(get(hb)))&LMASK)));
          Q.push_back(helse);
          filled[helse] = true;
          kid = nxt->at(kid);
        }
...
void Othello<keyType>::setAlienPreference(double ideal) {
  ...
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < L; j++) sa[j][i] = sb[j][i] = 0;
  ...
  for (int bitID = 0; bitID < L; bitID++) {
    ...
    for (int dir = 0; dir < 8 * 4; dir++) {
      ...
      int setB = ((dir & 16) != 0);
      double rate =
          getrate(ma, mb, na[bitID] + setA * emptyA + sa[bitID][dir & 0x7],
                  nb[bitID] + setB * emptyB + sb[bitID][dir & 0x7]);
  ...
  for (int i = 0; i < ma; i++)
    if (!filled[i]) set(i, veA);
  for (int i = ma; i < ma + mb; i++)
    if (!filled[i]) set(i, veB);
  ...
  for (int i = 0; i < ma + mb; i++) {
    ...
    valueType vv = 0;
    for (int bitID = 0; bitID < L; bitID++)
      if (da[direction[bitID] & 7] * diffa[bitID] +
              db[direction[bitID] & 7] * diffb[bitID] >
          0)
        vv |= (1 << bitID);
