...
#include <string>
/*!
 * \brief interface for converting a key from its raw format to a keyTypeype.
...
class IOHelper {
 public:
  ...
  /*!
   \brief split a keyTypeype value into two parts: groupID/keyInGroup by the
   highest *splitbit* bits.
  */
  virtual void splitgrp(const keyType &key, uint32_t &grp,
                        keyType &keyInGroup) = 0;
  /*!
...
class ConstantLengthKmerHelper : public IOHelper<keyType, valueType> {
 public:
  ...
  ConstantLengthKmerHelper(uint8_t _kmerlength, uint8_t _splitbit)
      : kmerlength(_kmerlength), splitbit(_splitbit){};
...
//! convert a 64-bit Integer to human-readable format in K/M/G. e.g, 102400 is
//! converted to "100K".
std::string human(uint64_t word) {
  ...
  if (word <= 1024)
    ...
    ss << std::setprecision(2) << word * 1.0 / 1024 << "K";
  else if (word <= 1048576)
    ss << word / 1024 << "K";
  else if (word <= 10485760)
    ss << word * 1.0 / 1048576 << "M";
  else if (word <= (1048576 << 10))
    ss << word / 1048576 << "M";
  else
    ss << word * 1.0 / (1 << 30) << "G";
  std::string s;
...
class KmerFileReader : public FileReader<keyType, valueType> {
 ...
 public:
  ...
  void finish() { fclose(f); }
  void reset() { rewind(f); }
  bool getFileIsSorted() { return fIsSorted; }
  ~KmerFileReader() { finish(); }
...
struct KVpair {
  keyType k;
  valueType v;
  bool friend operator<(const KVpair &a, const KVpair &b) { return a.k > b.k; }
...
class compressFileReader : public FileReader<keyType, valueType> {
 ...
 public:
  compressFileReader(const char *fname, IOHelper<keyType, valueType> *_helper,
                     uint32_t klength, uint32_t valuelength,
                     bool _fIsSorted = true) {
    curr = 0;
    max = 0;
    kl = klength;
    vl = valuelength;
...
class MultivalueFileReaderWriter : public FileReader<keyType, valueType> {
 ...
 public:
  ...
  MultivalueFileReaderWriter(const char *fname, uint32_t klength,
  ...
  void reset() { rewind(f); }
  bool getFileIsSorted() { return false; }
  ~MultivalueFileReaderWriter() { finish(); }
  void getmore() {
...
class SortedKmerTxtReader : public KmerReader<keyType> {
 ...
 public:
  ...
  ~SortedKmerTxtReader() {
    finish();
    if (binaryReader) delete binaryReader;
  }
  ...
  void finish() {
    if (binaryReader) binaryReader->finish();
  }
};
...
class taxoTreeBuilder : public FileReader<keyType, valueType> {
  ...
  struct KIDpair {
    ...
    bool friend operator<(const KIDpair &a, const KIDpair &b) {
      if (a.finished != b.finished)
        return (((int)a.finished) > ((int)b.finished));
      return a.k > b.k;
    }
  };
 ...
 public:
  ...
  void groupFile(string fname, vector<string> lf, string prefix, string suffix,
    ...
    for (string s : lf) {
      ...
      if (useBinaryKmerFile)
        ...
        readers.push_back(
            new SortedKmerTxtReader<keyType>(fname.c_str(), KmerLength, NULL));
      }
      keyType key;
    ...
    MultivalueFileReaderWriter<keyType, uint16_t> *writer =
        new MultivalueFileReaderWriter<keyType, uint16_t>(fname.c_str(), 8, 2,
                                                          false);
  ...
  taxoTreeBuilder(const char *NCBIfname, const char *fnameprefix,
    ...
    // Species_index	Species_ID	Species_name	Genus_index	Genus_ID
    // Genus_name Family_index	Family_ID	Family_name	Order_index
    // Order_ID	Order_name Class_index	Class_ID	Class_name
    // Phylum_index	Phylum_ID Phylum_name
    char buf[4096];
    ...
    for (int i = 0; i < levelcount; i++)
      localshift.push_back(
          localshift[i] +
          *max_element(NCBI_local[i].begin(), NCBI_local[i].end()) + 1);
    ...
    if (combineMode) {
      ...
      while (curr < fnames.size()) {
        if (curr + nn < fnames.size())
          fnamesInThisgrp = new vector<string>(fnames.begin() + curr,
                                               fnames.begin() + curr + nn);
        else
          fnamesInThisgrp =
        ...
        printf("merge kmer files %d %d to grp %s\n", curr,
               curr + fnamesInThisgrp->size() - 1, fnamegrp.c_str());
        groupFile(fnamegrp, *fnamesInThisgrp, prefix, suffix, curr,
                  useBinaryKmerFile, KmerLength, tmpFileDirectory);
      ...
      for (int i = 0; i < NCBI_ID.size(); i++) {
        ...
        if (useBinaryKmerFile)
          ...
          readers.push_back(new SortedKmerTxtReader<keyType>(
              fname.c_str(), KmerLength, tmpfname.c_str()));
        }
        keyType key;
        readers[readers.size() - 1]->getNext(&key);
  ...
  ~taxoTreeBuilder() {
    if (combineMode) {
      for (int i = 0; i < grpreaders.size(); i++) delete grpreaders[i];
    } else
      for (int i = 0; i < readers.size(); i++) delete readers[i];
