...
// split(k,plow,phigh,spl) (((plow)=(k & ((1ULL << spl)-1))), ((phigh)=(k >>
// spl)))

/*!
 * \brief Describes the data structure *Grouped l-Othello*. It classifies keys
 * of *keyType* into *2^L* classes. \n The keys are first classifed into groups,
 * each group is then maintained by one *l-Othello*. \note Query a key of
 * keyType always return uint64_t, however, only the lowest L bits are
 * meaningful. \n
 */
...
class MulOth {
  ...
  bool addOth(unsigned int groupid, vector<keyType> &keys, vector<VT> &values) {
    ...
    poth = new Othello<keyType>(L, &keys[0], keys.size(), true, &values[0],
                                sizeof(values[0]));
    if (!poth->build) {
      printf("Build Halt!\n");
      return false;
 ...
 public:
  vector<keyType>
      removedKeys;  //!< list of skipped keys for all underlying *Othello*.
  bool buildsucc;
  ...
  /*!
   ...
   \param unsigned char _split  the keys are first classifed according to their
   highest *_split* bits. There are in total 2^_split groups. Classification
   method as described in *splitgrp*. \param IOHelper _helper identifies how to
   convert a raw data to keytype and group. \param bool fileIsSorted When
   fileIsSorted, assume that the file is sorted so that the keys appear in the
   ascending order of groups.
   * */
  MulOth(uint32_t _L, unsigned char _split,
         class FileReader<keyType, uint16_t> *_reader) {
  ...
  MulOth(uint32_t _L, const char *fname, unsigned char _split,
         class IOHelper<keyType, IOvalueType> *_helper,
         bool fileIsSorted = false)
      : MulOth(_L, _split,
               new KmerFileReader<keyType, IOvalueType>(fname, _helper,
                                                        fileIsSorted)) {}
  ...
  void writeToFile(const char *fname) {
    ...
    for (int i = 0; i < (1 << split); i++) {
      if (vOths[i] != NULL) vOths[i]->writeDataToBinaryFile(pFile);
      //           fwrite(&(vOths[i]->mem[0]),sizeof(vOths[i]->mem[0]),
      //           vOths[i]->mem.size(), pFile);
    }
  ...
  MulOth(const char *fname, IOHelper<keyType, IOvalueType> *_helper)
    ...
    for (int i = 0; i < (1 << split); i++) {
      ...
      uint64_t *ppoth;
      ppoth = (uint64_t *)&buf0x20;
      if (*ppoth) poth = new Othello<keyType>(buf0x20);
    ...
    for (int i = 0; i < (1 << split); i++) {
      ...
      // fread(&(vOths[i]->mem[0]),sizeof(vOths[i]->mem[0]),
      // vOths[i]->mem.size(), pFile);
    }
    uint32_t nRemovedKeys = removedKeys.size();
